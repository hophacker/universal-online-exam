<?php
	...
	$listbox->AllowMultiSelect = true;//Use Multi selection is on.	
...
<form id="form1" method="post">
	...
	<div>
	Select the country that have you been to?
	</div>
	...
	<div style="float:left;padding-top:10px;padding-left:30px;">
		<input type="submit" value="Get Selected Items" />
		<br />
		<br />
		<?php 
			if(count($listbox->SelectedItems)>0)
			{
				echo "You have been to: ";
				for($i=0;$i<count($listbox->SelectedItems);$i++)
				{
					echo "[".$listbox->SelectedItems[$i]->Value."] ";
				}
			}
		?>		
	</div>
